...
function hyStrNum(str) --自定义的点击函数
...
function bosssign( ... ) --记录BOSS出现时间以及初始化变量sing_boss
        if multiColor( ... ) then
                bossTime = os.time()
                sign_boss = 1
        end
...
local w,h = getScreenSize() --获取当前屏幕的宽高(w，h),并且复制给变量w,h
function hrmove( ... )
        if w==640 and h==1136 then
        --蓝色按钮打开状态，武器按钮
                if multiColor({{  204, 1092, 0xffffff},{  234, 1098, 0xffffff}}) and multiColor({{ 78,1020,0xc46212}}) ==false then
                        moveTo( ... )
                        mSleep(50)
                end
...
UINew("丸子疯狂帮你打怪兽","运行脚本","立即退出","crazymonster.dat",0,30) --UI开始
...
if func3 == "使用主角技能" then --如果勾选了UI界面中的'使用主角技能'
        if skillcd == "10分钟" then --那就判断UI界面中设定的技能CD时长
                cd = 600
...
if func1 == "自动打怪" then --如果勾选了UI界面中的'自动打怪'功能
        if step == "每秒40下" then --那就判断UI界面中设定的屏幕点击次数
                tapms = 15
                toast("自动打怪：每秒40下")
        ...
        elseif step == "自定义速度(安卓无效)" then
                if getOSType() =="ios" then --如果获取到当前设备的系统是IOS 那么就启用hyStrNum()设定点击速度
...
local bossTime = os.time() --初始化bossTime函数的时间
local multime = os.time() --初始化变量multime的时间
local yfstime = 200--影分身
while (true) do --程序运行主体
        flag = deviceIsLock() --先看看设备有没有锁定，devicelsLock()会传回一个值给变量flag
        if flag ~= 0 then --如果锁屏了就解锁
                unlockDevice()
        end
        if func1 == "自动打怪" then
                local t1 = os.time()
        ...
        if sign_boss ~= 1 then--非boss 战斗状态
                ...
                if func2 == "解锁升级最新英雄" then
                        if w==640 and h==1136 then --下面就是对各个分辨率下的找色点击适配
                --无法击败(窗口未适配)
                                multiColTap({{ 389, 637,0x2c2930},{ 383, 670,0x4f4856},{ 383, 722,0x06688d},{ 452, 721,0x06688d}})
                --关闭武器界面
                                multiColTap({{ 323,  181, 0x4f4856},{ 426,  188, 0x4f4856},{ 555,  211, 0x2c2931},{ 564,  202, 0x2c2931}}) 
                --关闭英雄详情界面
                                multiColTap({{ 119,  171, 0x4f4856},{ 136,  393, 0x4f4856},{ 555,  219, 0x2c2931},{ 565,  209, 0x2c2931}}) 
                                if multiColor({{  204, 1092, 0xffffff},{  234, 1098, 0xffffff},{  535,  707, 0xeeaa17}}) then
                --武器左边剑一蓝
                                        upgrade({{ 78,1020,0xc46212},{260, 1031, 0xeeaa17},{ 566,900,0x218db0},{ 566, 900,0x218db0}}) 
                --武器左边剑武器一黄
                                        upgrade({{ 78,1020,0xc46212},{260, 1031, 0xeeaa17},{ 585, 900,0xeeaa17},{ 585, 900,0xeeaa17}}) 
                --武器一灰二蓝
                                        upgrade({{ 77, 1008, 0xc46212},{589, 908,0x787686},{585,791,0x218db0},{566,795,0x218db0}}) 
                 --武器二黄 
                                        upgrade({{ 77, 1008, 0xc46212},{585,791,0xeeaa17},{585,791,0xeeaa17}})     
                                else
                                        multiColTap({{  204, 1092, 0x16628a},{  234, 1098, 0x115a80},{  184, 1113, 0x42b1eb}})
                                end
                --挑战魔王，白色小手两点、重新挑战橙色按钮
                                boss({{ 548, 326,0xffffff},{ 554, 345,0xffffff},{ 540, 269,0xdd5e0a},{ 540, 269,0xdd5e0a}}) 
                --是否打boss，红色退出按钮
                                bosssign({{ 502, 265,0xdd3a15},{ 588, 267,0xdd3a15}}) 
                        elseif w==750 and h==1334 then
                        ...
                        elseif w==1536 and h==2048 then
                                multiColTap({{  284,  426, 0x4f4856},{  634,  442, 0xf74e53},{  908,  438, 0xf74e53},{ 1232,  462, 0x2c2931}})
                --关闭公告窗口
                                multiColTap({{  230,  665, 0xc23c00},{  259,  664, 0xb61e00},{  249,  671, 0xffe129},{  263,  728, 0xd44302}})
                --离线奖励
                                multiColTap({{  206,  168, 0x4f4856},{  272, 1858, 0x5c4c6b},{  892, 1918, 0xb63924},{ 1288,  210, 0x2c2931}})
                --好友列表
                                multiColTap({{  192,  163, 0x605967},{  166,  606, 0x605967},{ 1198,  222, 0x605967},{ 1276,  186, 0x2c2931}})
                --世界排行
                                multiColTap({{ 389, 637,0x2c2930},{ 383, 670,0x4f4856},{ 383, 722,0x06688d},{ 452, 721,0x06688d}}) 
                --无法击败(窗口未适配)
                                multiColTap({{  316,  228, 0x4f4856},{  753,  313, 0x4f4856},{ 1215,  301, 0x2c2931},{ 1234,  285, 0x2c2931}}) 
                --关闭武器界面
